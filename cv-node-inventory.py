...
def delete_nodeconfig_for_node(node_name: str, location: str) -> bool:
    """Delete NodeConfig from CloudVision API.

    Args:
        node_name: Name of the node to delete
        location: Location identifier (cluster name)
    ...
    while True:
        ...
        if removed_nodes:
            ...
            for node_name in sorted(removed_nodes):
                if delete_nodeconfig_for_node(node_name, cluster_name):
                    success_count += 1
                else:
                    failure_count += 1
